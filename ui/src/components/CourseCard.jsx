import React, { useState } from 'react'
import image1 from '../assets/images/rp.png'
import { Link } from 'react-router-dom'

const CourseCard = ({course}) => {

const [showFullDesc,setShowFullDesc]= useState(false)

let desc=course.description

if (!showFullDesc){
  desc=desc.substring(0,100)
}

const toggleText=() =>{
  setShowFullDesc(!showFullDesc)
}




return (
    <>
 <div className=' bg-purple-100  rounded-md shadow-2xl flex flex-col items-center justify-center mx-5 my-5 py-10'>
          <h2 className=' font-bold text-lg text-purple-900 '>{course.title}</h2>
          <img src={image1} alt="course thumbnail" className='w-80 h-40 ' />

          <p className='text-black group-hover:text-white my-2 mx-5'> {desc}</p>
          <button onClick={toggleText}> {showFullDesc? 'less': 'more'} </button>
          <Link to={`/courses/${course.courseId}`} className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600 self-start mx-5">Learn More</Link>
        </div>
    </>
  )
}

// return (
//   <>
// <div className=' bg-purple-100  rounded-md shadow-2xl flex flex-col items-center justify-center mx-5 my-5 py-10'>
//         <h2 className=' font-bold text-lg text-purple-900 '>{course.title}</h2>
//         <img src={image1} alt="course thumbnail" className='w-80 h-40 ' />

//         <p className='text-black group-hover:text-white my-2 mx-5'> {desc}</p>
//         <button onClick={toggleText}> {showLess? 'more': 'less'} </button>
//         <a href="#" className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600 self-start mx-5">Learn More</a>
//       </div>
//   </>
// )
// }


export default CourseCard